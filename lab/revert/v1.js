...
function mistHandoff() {
  ...
  const plates = [...fog.querySelectorAll('[data-fog-plate]')];
  const bathImg = bath.querySelector('.bath__img');
  const line = bath.querySelector('[data-bath-line]');

  if (REDUCED) {
    gsap.set([bath, line], { opacity: 1 });
    gsap.set(line, { y: 0 });
    return;
  }

  // The scene rises at exactly the speed the fog leaves, so the two read as
  // one mass travelling through the frame rather than as a curtain fading
  // off a picture. Both use RISE_BY over the same window, so the lock holds
  // by construction rather than by arithmetic anyone can get wrong.
  //
  // RISE_BY is capped by the 20% of headroom .bath__img carries in CSS.
  // Raising it means giving the image more headroom, which costs more of
  // its width, because we chose not to make a taller source.
  const RISE_AT = 0.42;
  const RISE_END = 0.97;

  // Three quarters of a viewport, where it used to be 0.17. The old cap was
  // the headroom needed to keep the scene's top edge out of frame. Feather
  // that edge instead and the cap disappears, so the climb can be a real
  // move rather than a nudge.
  const RISE_BY = 0.75;

  // Light, because it is atmosphere now rather than a cover. With the hero
  // pinned there is no moving edge to hide, and the scene's own top edge is
  // feathered rather than hard, so nothing here is load bearing. At 0.8 the
  // room stays plainly readable through it, which is the difference between
  // thick weather and a white screen.
  const VEIL_PEAK = 0.80;

  gsap.set(fog, { opacity: 1 });
  gsap.set(veil, { opacity: 0 });
  gsap.set(plates, { opacity: 0 });
  gsap.set(bath, { opacity: 0 });
  gsap.set(bathImg, { y: () => window.innerHeight * RISE_BY });
  gsap.set(line, { opacity: 0, y: 18 });

  const vh = () => window.innerHeight;

  // The hero holds still for the whole of the covering, so the join
  // between the two sections never moves while it is being hidden. It is
  // released only once the veil is solid, and by then the bath's own
  // sticky has taken over the frame, so nothing about the swap is on
  // screen to see.
  ScrollTrigger.create({
    trigger: '.act--hero',
    start: 'top top',
    ...
    pin: heroStage,
    pinSpacing: true,
    // First pin on the page, so it refreshes first and every pin below it
    // measures against a spacer that already exists.
    refreshPriority: 3,
    invalidateOnRefresh: true
  });
  ...
  // Two phases, because the two halves want opposite things.
  //
  // Arriving, the smoke has to visibly climb the room, and the dense part
  // of the art sits in the lower third of the plate, so it needs real
  // distance to bring that structure up into frame. A whole viewport of it.
  // Running the same small locked rate here was why the smoke read as a
  // haze at the bottom edge instead of as weather coming up the wall.
  //
  // Leaving, it has to travel at exactly the scene's rate, so the two read
  // as one mass. Nothing before RISE_AT is locked to anything, which is the
  // freedom that lets the arrival be quick and the exit be matched.
  plates.forEach(el => {
    const from = parseFloat(el.dataset.from);
    const mid = parseFloat(el.dataset.mid);
    tl.fromTo(el,
      { y: () => vh() * from },
      { y: () => vh() * mid, ease: 'none', duration: RISE_AT },
      0)
      .to(el,
        { y: () => vh() * (mid - RISE_BY), ease: 'none', duration: RISE_END - RISE_AT },
        RISE_AT);
  });

  // The plates lead and the veil follows, so fog is seen arriving before
  // the frame goes dense. Reversed on the way out: the veil leaves first
  // so the bath appears, and the plates linger a beat over it, which is
  // the wisps-over-the-scene detail the reference ends on.
  plates.forEach(el => {
    tl.to(el, { opacity: parseFloat(el.dataset.peak) || 1, duration: 0.14, ease: 'none' }, 0.02)
      // Out after the veil and across the back half of the climb, so the
      // last thing on screen is wisps travelling up off a room that is
      // still settling, rather than a clean switch.
      .to(el, { opacity: 0, duration: 0.33, ease: 'none' }, 0.62);
  });

  // Held back deliberately. Smoke is only visible as contrast against
  // something, so while the veil is building it is removing the very
  // backdrop that makes the smoke legible. Letting the plate arrive alone
  // first buys a window where what is on screen is actual smoke over an
  // actual room, which is the only place its shape can be read at all.
  tl.to(veil, { opacity: VEIL_PEAK, duration: 0.14, ease: 'none' }, 0.20)
    // The scene is stuck flush well before this, and its image is still
    // parked below the fold, so this is switching on something nobody can
    // see. It lands just under the fog's densest point, and the climb
    // starts as it finishes, so what the reader sees is the scene arriving
    // by moving rather than by appearing.
    .to(bath,  { opacity: 1, duration: 0.06, ease: 'none' }, 0.36)
    // The rise and the clearing run on the same window and the same linear
    // rate, so the fog leaving the top and the room arriving from below are
    // one continuous movement.
    .fromTo(bathImg,
      { y: () => vh() * RISE_BY },
      { y: 0, ease: 'none', duration: RISE_END - RISE_AT }, RISE_AT)
    // Out across the first half of the climb. It can leave early because
    // the scene is covering more of the frame every moment, and what is
    // still exposed above the feather has the fog's own dense band sitting
    // on it, which travels with the junction by construction.
    .to(veil,  { opacity: 0, duration: 0.30, ease: 'none' }, RISE_AT)
